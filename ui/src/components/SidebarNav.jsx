...
const SidebarNav = ({ activeTab, onChange }) => {
  ...
  return (
    <aside className="scanline relative flex w-full flex-col bg-[#060D1F] text-white lg:sticky lg:top-0 lg:h-screen lg:w-[280px] lg:flex-shrink-0 lg:overflow-y-auto">
      ...
      <div className="border-b border-cyan-900/40 px-6 py-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-xl font-bold tracking-tight text-white">SOX Agent</div>
            <p className="mt-0.5 text-sm leading-4 text-cyan-400/80">ITGC Compliance AI</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 space-y-8 px-3 py-5">
        {groups.map((group) => (
          <div key={group.label}>
            <div className="px-3 text-xs uppercase tracking-[0.24em] text-cyan-500/60">{group.label}</div>
              ...
              {group.items.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`sidebar-item-enter flex w-full items-center gap-3.5 rounded-lg px-4 py-4 text-left text-lg font-medium transition-all duration-200 hover:translate-x-1 ${
                      isActive
